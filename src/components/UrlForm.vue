  ...
  methods: {
    shorten() {
      if (this.inputUrl) {
        ...
        this.isError = true;
        this.errorMessage =
          "Please ensure to fill out the url field before using the url shortener.";
      }
    },
    shorternerLogic() {
      const validUrl =
        this.inputUrl.includes("https://") || this.inputUrl.includes("http://");
      const fullUrl = validUrl
        ? new URL(this.inputUrl)
        : new URL(`https://${this.inputUrl}`);
      const shortUrl = `${fullUrl.protocol}//${shortid.generate()}`;
      this.urlObject = { fullUrl: fullUrl.href, shortUrl, copied: false };

      if (
        this.urls.filter((url) => url.fullUrl === fullUrl.href).length === 0
      ) {
        this.urls.push(this.urlObject);
      } else {
        this.isError = true;
        this.errorMessage =
          "This url has already been submitted, duplicates are rejected.";
      }
    },
    onCopy(index) {
